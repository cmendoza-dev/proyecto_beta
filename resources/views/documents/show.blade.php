                                ...
                                @if (auth()->user()->role === 'Secretary' || auth()->user()->role === 'Administrator')
                                    <form action="{{ route('documents.destroy', $document) }}" method="POST"
                                        ...
                                        <button type="submit" class="text-red-600 hover:text-red-800"
                                            onclick="event.preventDefault(); if (typeof Swal === 'undefined') { if (confirm('¿Eliminar este documento?')) { this.closest('form').submit(); } return; } Swal.fire({ title: '¿Eliminar este documento?', text: 'Esta acción no se puede deshacer.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#6b7280', confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' }).then((result) => { if (result.isConfirmed) { this.closest('form').submit(); } });"
                                            title="Eliminar">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
