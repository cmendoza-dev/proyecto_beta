...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('meetings.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">
    ...
    <div class="p-6 bg-white rounded-lg shadow">
        <form method="GET" action="{{ route('meetings.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route('meetings.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300">
                    Limpiar
                </a>
            </div>
        </form>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center text-sm text-gray-600">
                            ...
                            {{ $meeting->opening_time }} - {{ $meeting->closing_time }}
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-4 mt-4 border-t border-gray-200">
                        <a href="{{ route('meetings.show', $meeting) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">
                            Ver detalles →
                        </a>
                        ...
                        <div class="flex items-center space-x-2">
                            @if($meeting->status === 'draft')
                                <form method="POST" action="{{ route('meetings.open', $meeting) }}">
                                    @csrf
                                    <button type="submit" class="p-2 text-green-600 hover:bg-green-50 rounded-lg" title="Abrir reunión">
                            ...
                            @if($meeting->status === 'open')
                                <form method="POST" action="{{ route('meetings.close', $meeting) }}" x-data @submit.prevent="if(confirm('¿Cerrar esta reunión? No se podrán registrar más asistencias.')) $el.submit()">
                                    @csrf
                                    <button type="submit" class="p-2 text-red-600 hover:bg-red-50 rounded-lg" title="Cerrar reunión">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="{{ route('meetings.edit', $meeting) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Editar">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @empty
            <div class="col-span-full">
                <div class="py-12 text-center bg-white rounded-lg shadow">
                    ...
                    <a href="{{ route('meetings.create') }}" class="inline-block px-6 py-3 mt-6 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">
